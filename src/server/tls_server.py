...
def main():
    ...
    with socket(AF_INET, SOCK_STREAM) as server_socket:
        ...
        with context.wrap_socket(server_socket, server_side=True) as tls_socket:
            logger.info("TLS server listening on 0.0.0.0:%d", server_port)
            logger.info("Waiting for client connections...")

            try:
                shutdown = False
                while not shutdown:
                    conn, client = tls_socket.accept()
                    logger.info("Connection established with %s:%d", *client)
                    logger.info(
                        "TLS handshake with %s:%d succeeded — server certificate accepted by client (version=%s, cipher=%s)",
                        *client,
                        conn.version(),
                        conn.cipher()[0],
                    )
